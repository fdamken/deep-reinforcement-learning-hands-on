...
if __name__ == "__main__":
    ...
    # Collection of data ofr monitoring.
    gen_losses = []
    dis_losses = []
    iter_no = 0
    ...
    for batch_v in iterate_batches(envs):
        ...
        if iter_no % REPORT_EVERY_ITER == 0:
            # Use the means and reset the collected losses to get accurate monitoring over REPORT_EVERY_ITER amount of batches.
            gen_loss_mean = np.mean(gen_losses)
            dis_loss_mean = np.mean(dis_losses)
            gen_losses = []
            dis_losses = []

            log.info('Iter %d: gen_loss=%.3f, dis_loss=%.3f', iter_no, gen_loss_mean, dis_loss_mean)
            writer.add_scalar('gen_loss', gen_loss_mean, iter_no)
            writer.add_scalar('dis_loss', dis_loss_mean, iter_no)
