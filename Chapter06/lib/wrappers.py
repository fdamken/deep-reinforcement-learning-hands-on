...
class ResizeFrame(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        return ResizeFrame.process(observation)
...
class BufferWrapper(gym.ObservationWrapper):
    def __init__(self, env, n_steps, dtype = np.float32):
        ...
        self._buffer = np.zeros_like(self.observation_space.low, dtype = self._dtype)
    ...
    def reset(self):
        self._buffer = np.zeros_like(self._buffer, dtype = self._dtype)
